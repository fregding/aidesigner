...
if (imageConfig.providerType === 'pollinations') {
  ...
  static getImageProviderConfigs(runtimeConfig = {}) {
   ...
   if (this.isPollinationsImageProvider(runtimeConfig)) {
    ...
    const fallbackProvider = String(
      ...
      runtimeConfig.imageFallbackProvider ||
      ''
    ).trim().toLowerCase();
    const fallbackBaseUrlRaw = process.env.IMAGE_FALLBACK_BASE_URL || runtimeConfig.imageFallbackBaseUrl || '';
    const fallbackApiKey = process.env.IMAGE_FALLBACK_API_KEY || runtimeConfig.imageFallbackApiKey || runtimeConfig.anthropicFallbackApiKey || runtimeConfig.providerApiKey || '';
    const fallbackModel = process.env.IMAGE_FALLBACK_MODEL || runtimeConfig.imageFallbackModel || '';
    const fallbackIsPollinations = fallbackProvider === 'pollinations' || fallbackProvider === 'pollinations.ai' || this.isPollinationsBaseUrl(fallbackBaseUrlRaw);

    if (fallbackEnabled && fallbackIsPollinations) {
      const fallbackConfig = this.getPollinationsImageRuntimeConfig(runtimeConfig, 'fallback', {
        baseUrl: fallbackBaseUrlRaw || 'https://image.pollinations.ai',
        apiKey: fallbackApiKey,
        model: fallbackModel || 'flux'
      });
    ...
    const fallbackBaseUrl = this.normalizeOpenAiCompatibleBaseUrl(fallbackBaseUrlRaw || '');
  ...
  static getImageRouterCandidates(runtimeConfig = {}) {
    // Local OpenVINO and Pollinations are direct image providers, not model-router text profiles.
    // Disable router candidates so IMAGE_BASE_URL/IMAGE_FALLBACK_* are honored exactly.
    if (this.isLocalImageMode(runtimeConfig) || this.isOpenAiCompatibleImageMode(runtimeConfig) || this.isPollinationsImageProvider(runtimeConfig)) {
      return [];
    }
    try {
  ...
  static imageRuntimeConfigFromCandidate(candidate = {}, runtimeConfig = {}) {
    if (this.isLocalImageMode(runtimeConfig)) {
      return this.getLocalImageRuntimeConfig(runtimeConfig, candidate.role || candidate.channel || 'local');
    }
  ...
  static getImageRuntimeConfig(runtimeConfig = {}, channel = 'primary') {
    if (this.isLocalImageMode(runtimeConfig)) {
      return this.getLocalImageRuntimeConfig(runtimeConfig, channel === 'fallback' ? 'local-fallback' : 'local');
    }
...
static isPollinationsBaseUrl(baseUrl = '') {
  return /(^|\.)pollinations\.ai/i.test(String(baseUrl || ''));
}

static getPollinationsImageRuntimeConfig(runtimeConfig = {}, channel = 'primary', overrides = {}) {
  const isFallback = channel === 'fallback';
  return {
    id: isFallback ? 'pollinations-fallback' : 'pollinations',
    name: isFallback ? 'Pollinations Fallback' : 'Pollinations',
    channel,
    providerType: 'pollinations',
    baseUrl: this.normalizePlainBaseUrl(
      overrides.baseUrl ||
      (isFallback ? (process.env.IMAGE_FALLBACK_BASE_URL || runtimeConfig.imageFallbackBaseUrl) : '') ||
      process.env.IMAGE_BASE_URL ||
      runtimeConfig.imageBaseUrl ||
      'https://image.pollinations.ai'
    ),
    apiKey: overrides.apiKey ?? (isFallback
      ? (process.env.IMAGE_FALLBACK_API_KEY || runtimeConfig.imageFallbackApiKey || '')
      : (process.env.IMAGE_API_KEY || runtimeConfig.imageApiKey || '')),
    model: overrides.model || (isFallback
      ? (process.env.IMAGE_FALLBACK_MODEL || runtimeConfig.imageFallbackModel || 'flux')
      : (process.env.IMAGE_MODEL || runtimeConfig.imageModel || 'flux')),
    quality: runtimeConfig.imageQuality || 'standard',
    timeoutMs: Math.max(
