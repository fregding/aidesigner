...
const CONFIG_DEFINITIONS = [
  {
    ...
    type: 'text',
    description: '统一 AI 服务地址，例如 http://127.0.0.1:18080/v1',
    defaultValue: process.env.TIME_BACKWARD_BASE_URL || 'http://127.0.0.1:18080/v1',
    hidden: true
  },
  {
  ...
  },
  {
    ...
    type: 'text',
    description: 'Claude/Anthropic Messages 主通道失败时自动切换的备用服务地址，本地图片版默认留空',
    defaultValue: process.env.ANTHROPIC_FALLBACK_BASE_URL || process.env.TIMEBACKWARD_FALLBACK_BASE_URL || '',
    hidden: true
  },
  {
  ...
  },
  {
    ...
    label: '图片助手 Base URL',
    type: 'text',
    description: '图片页 AI 助手使用的 OpenAI 兼容地址，例如 http://127.0.0.1:18080/v1',
    defaultValue: process.env.IMAGE_ASSISTANT_BASE_URL || process.env.IMAGE_BASE_URL || 'http://127.0.0.1:18080/v1',
    hidden: true
  },
  {
  ...
  },
  {
    ...
    type: 'text',
    description: '图片 AI 助手备用 OpenAI 兼容地址，例如 http://127.0.0.1:18080/v1',
  ...
  },
  {
    ...
    defaultValue: process.env.IMAGE_BASE_URL || process.env.TIMEBACKWARD_IMAGE_BASE_URL || 'http://127.0.0.1:18080/v1',
    hidden: true
  },
  {
    key: 'image_api_key',
    ...
    defaultValue: process.env.IMAGE_API_KEY || process.env.TIMEBACKWARD_IMAGE_API_KEY || 'local-ai-dev-key',
    hidden: true
  },
  {
  ...
  },
  {
    ...
    label: '图片备用 Base URL',
    type: 'text',
    description: '图片生成备用 OpenAI 兼容地址，例如 http://127.0.0.1:18080/v1',
    defaultValue: process.env.IMAGE_FALLBACK_BASE_URL || process.env.TIMEBACKWARD_IMAGE_BASE_URL || '',
    hidden: true
  },
  {
  ...
  },
  {
    ...
    defaultValue: process.env.IMAGE_MODEL || 'local-sd15-openvino',
  ...
  },
  {
    ...
    defaultValue: process.env.IMAGE_QUALITY || 'low',
    options: [
      { label: '低', value: 'low' },
  ...
  },
  {
    ...
    defaultValue: process.env.IMAGE_TIMEOUT_MS || '1800000'
  },
  {
  ...
  },
  {
    ...
    defaultValue: process.env.REGISTRATION_INITIAL_CREDITS || '999999999'
  },
  {
  ...
  },
  {
    ...
    defaultValue: process.env.EMAIL_VERIFICATION_ENABLED || 'false'
  },
  {
    key: 'smtp_host',
    group: 'email',
  ...
  },
  {
    ...
    defaultValue: process.env.SITE_RECHARGE_MODE || 'card_link',
    options: [
...
class RuntimeConfigService {
  ...
  static getAdminConfigs() {
    ...
    return CONFIG_DEFINITIONS
      ...
      .filter(definition => {
        if (!appConfig.imageOnlyMode) return true;
        return !['payment', 'video', 'search', 'ppt_agent'].includes(definition.group);
      })
      .map(definition => {
  ...
  static getRuntimeConfig(overrides = {}) {
    ...
    if (appConfig.imageOnlyMode) {
      raw.provider_base_url = process.env.TIME_BACKWARD_BASE_URL || 'http://127.0.0.1:18080/v1';
      raw.provider_api_key = process.env.TIME_BACKWARD_API_KEY || '';
      raw.anthropic_fallback_base_url = '';
      raw.anthropic_fallback_api_key = '';
      raw.image_base_url = process.env.IMAGE_BASE_URL || 'http://127.0.0.1:18080/v1';
      raw.image_api_key = process.env.IMAGE_API_KEY || 'local-ai-dev-key';
      raw.image_model = process.env.IMAGE_MODEL || 'local-sd15-openvino';
      raw.image_quality = process.env.IMAGE_QUALITY || 'low';
      raw.image_output_format = process.env.IMAGE_OUTPUT_FORMAT || 'png';
      raw.image_timeout_ms = process.env.IMAGE_TIMEOUT_MS || '1800000';
      raw.image_failover_enabled = 'false';
      raw.image_fallback_base_url = '';
      raw.image_fallback_api_key = '';
      raw.image_fallback_model = '';
      raw.image_provider_id = '';
      raw.image_fallback_provider_ids = '';
      raw.image_assistant_base_url = '';
      raw.image_assistant_api_key = '';
      raw.image_assistant_provider_id = '';
      raw.image_assistant_fallback_provider_ids = '';
      raw.video_provider_id = '';
      raw.video_fallback_provider_ids = '';
      raw.search_api_key = '';
      raw.search_base_url = '';
      raw.email_verification_enabled = 'false';
      raw.registration_initial_credits = process.env.REGISTRATION_INITIAL_CREDITS || String(appConfig.localFullCredits || 999999999);
      raw.site_recharge_mode = 'card_link';
      raw.alipay_enabled = 'false';
    }
    ...
    return {
      providerBaseUrl: this.normalizeBaseUrl(primaryTextProvider?.baseUrl || raw.provider_base_url, appConfig.imageOnlyMode ? 'http://127.0.0.1:18080/v1' : 'https://llmapi.pro'),
      ...
      imageBaseUrl: this.normalizeOpenAiBaseUrl(imageProvider?.baseUrl || raw.image_base_url || 'http://127.0.0.1:18080/v1'),
      imageApiKey: imageProvider?.apiKey || raw.image_api_key || raw.anthropic_fallback_api_key || raw.provider_api_key || (appConfig.imageOnlyMode ? 'local-ai-dev-key' : ''),
      ...
      imageFallbackApiKey: raw.image_fallback_api_key || raw.anthropic_fallback_api_key || raw.provider_api_key,
      imageFallbackModel: raw.image_fallback_model || '',
      imageAssistantBaseUrl: this.normalizeBaseUrl(imageAssistantProvider?.baseUrl || raw.image_assistant_base_url || raw.image_base_url || 'https://timebackward.com/v1', ''),
      imageAssistantApiKey: imageAssistantProvider?.apiKey || raw.image_assistant_api_key || raw.image_api_key || raw.anthropic_fallback_api_key || raw.provider_api_key,
      ...
      imageModel: raw.image_model || imageProvider?.defaultModel || (appConfig.imageOnlyMode ? 'local-sd15-openvino' : 'gpt-image-2'),
      ...
      searchBaseUrl: this.normalizeBaseUrl(raw.search_base_url, appConfig.imageOnlyMode ? '' : 'https://api.tavily.com'),
  ...
  static buildDefaultTextProviderProfiles(raw = {}) {
    ...
    const profiles = [
      {
        ...
        baseUrl: raw.provider_base_url || process.env.TIME_BACKWARD_BASE_URL || (appConfig.imageOnlyMode ? 'http://127.0.0.1:18080/v1' : 'https://llmapi.pro'),
    ...
    if (raw.anthropic_fallback_base_url || raw.anthropic_fallback_api_key) {
      profiles.push({
        ...
        name: 'Anthropic 备用',
        format: 'anthropic',
        baseUrl: raw.anthropic_fallback_base_url || '',
    ...
    const imageAssistantBaseUrl = raw.image_assistant_base_url || raw.image_base_url || process.env.IMAGE_ASSISTANT_BASE_URL || process.env.IMAGE_BASE_URL || '';
    const imageAssistantApiKey = raw.image_assistant_api_key || raw.image_api_key || raw.anthropic_fallback_api_key || raw.provider_api_key || process.env.IMAGE_ASSISTANT_API_KEY || process.env.IMAGE_API_KEY || '';
    if (imageAssistantBaseUrl || imageAssistantApiKey) {
      profiles.push({
        id: 'image_assistant',
        name: '图片助手服务',
        format: 'openai',
        baseUrl: imageAssistantBaseUrl || (appConfig.imageOnlyMode ? '' : 'https://timebackward.com/v1'),
    ...
    if (imageAssistantFallbackBaseUrl || imageAssistantFallbackApiKey) {
      profiles.push({
        ...
        name: '图片助手备用',
        format: 'openai',
        baseUrl: imageAssistantFallbackBaseUrl || (appConfig.imageOnlyMode ? '' : 'https://timebackward.com/v1'),
  ...
  static normalizeOpenAiBaseUrl(value, fallback = (appConfig.imageOnlyMode ? 'http://127.0.0.1:18080/v1' : 'https://timebackward.com/v1')) {
