  ...
  try {
    ...
    res.json({
      stats: {
        totalUsers,
        completedTasks,
        toolCount: appConfig.features.image ? 1 : 0
      }
    });
  } catch (error) {
...
function disabledFeatureHandler(featureName) {
  return (req, res) => {
    res.status(410).json({
      error: `${featureName} 已在本地图片版中禁用`,
      local_image_only: true
    });
  };
}

if (appConfig.imageOnlyMode || !appConfig.features.ppt) {
  app.use([
    '/api/ai/generate/ppt',
    '/api/ai/generate/ppt-from-doc',
    '/api/ai/ppt/import',
    '/api/ai/convert/document',
    '/api/ai/convert/from-path'
  ], disabledFeatureHandler('PPT/文档能力'));
}

if (appConfig.imageOnlyMode || !appConfig.features.video) {
  app.use(['/api/ai/generate/video'], disabledFeatureHandler('视频生成能力'));
}

if (appConfig.imageOnlyMode || !appConfig.features.assistant) {
  app.use([
    '/api/ai/chat/assistant',
    '/api/ai/assistant/respond',
    '/api/ai/assistant/respond/jobs',
    '/api/ai/assistant/respond/stream'
  ], disabledFeatureHandler('外部 AI 助手能力'));
}
...
if (payRoutes) {
  ...
  app.use('/api/pay', disabledFeatureHandler('支付能力'));
}
...
app.get('/api/config', (req, res) => {
  ...
  res.json({
	      ...
	      payment: {
	        mode: runtimeConfig.rechargeMode,
	        alipayEnabled: Boolean(appConfig.features.payments && runtimeConfig.alipay?.enabled),
	        purchaseCardUrl: runtimeConfig.purchaseCardUrl || ''
	      }
	    },
    features: {
      ppt: Boolean(appConfig.features.ppt),
      image: Boolean(appConfig.features.image),
      video: Boolean(appConfig.features.video),
      payments: Boolean(appConfig.features.payments),
      assistant: Boolean(appConfig.features.assistant),
      localImageOnly: Boolean(appConfig.imageOnlyMode)
    },
    limits: {
