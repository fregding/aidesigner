...
class User {
  ...
  static buildQuotaSnapshot(user) {
    ...
    if (user.role === 'admin' && appConfig.imageOnlyMode) {
      const fullCredits = Math.max(1000000, Number(appConfig.localFullCredits || 999999999));
      const used = Math.max(0, Number(user.credits_used || 0));
      const remaining = Math.max(0, fullCredits - used);
      const universal = {
        total: fullCredits,
        used,
        remaining,
        permanent: { total: fullCredits, used, remaining },
        vip_bonus: { total: 0, used: 0, remaining: 0, expires_at: null, priority: 'first' }
      };
      return {
        credits: universal, universal, ppt: universal, image: universal, video: universal, chat: universal,
        legacy: {
          ppt: { total: fullCredits, used: user.used_ppt || 0, remaining: fullCredits - (user.used_ppt || 0) },
          image: { total: fullCredits, used: user.used_image || 0, remaining: fullCredits - (user.used_image || 0) },
          video: { total: fullCredits, used: user.used_video || 0, remaining: fullCredits - (user.used_video || 0) },
          chat: { total: fullCredits, used: user.used_chat || 0, remaining: fullCredits - (user.used_chat || 0) }
        }
      };
    }
  ...
  static checkQuota(id, type, amount = 1) {
    const user = this.refreshVipStatus(id);
    if (!user) return false;
    if (user.role === 'admin' && appConfig.imageOnlyMode) return true;
